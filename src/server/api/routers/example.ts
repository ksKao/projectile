...
export const exampleRouter = createTRPCRouter({
	...
	generateDemoData: protectedProcedure.mutation(async ({ ctx }) => {
		...
		const project = await ctx.db.$transaction(async (transaction) => {
			...
			// tasks
			await transaction.tasks.createMany({
				data: [
					{
			...
			//thread replies
			await transaction.threadReplies.createMany({
				data: [
					{
			...
			// poll options
			await transaction.pollOptions.createMany({
				data: [
					{
						pollsId: getPollId(0),
						sortOrder: 0,
